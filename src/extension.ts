...
export async function activate(context: vscode.ExtensionContext) {
	let disposable = vscode.commands.registerCommand('search-medium-ai-blog.searchMediumAiBlog', async () => {
		try {
		...
		const articles: IRssDataResponse[] = [...new Map(data.map(item => [item['link'], item])).values()];
